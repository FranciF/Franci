...
class CrimeMining:
    ...
    '''抽取出事件三元组'''
    def extract_triples(self, words, postags,sentence):
        svo = []
        tuples, child_dict_list = self.parser.parser_main(words, postags,sentence)
        for tuple in tuples:
            rel = tuple[-1]
            if rel in ['SBV']:
                sub_wd = tuple[1]
    ...
    def main(self, content):
        ...
        # 存储文章事件
        events = []
        print([(sent) for sent in subsents])
        for sent in subsents:
            ...
            if ner:
                print(sent)
                triple = self.extract_triples(words, postags,sent)
                if not triple:
                    continue
                triples += triple
                ners += ner
